...
@Component({
  selector: 'app-transterceroform',
  templateUrl: './transterceroform.page.html',
  styleUrls: ['./transterceroform.page.scss'],
})
export class TransterceroformPage implements OnInit {
  ...
  misCuentas() {
    this.FireAuth.authState.subscribe(user => {
    this.FireStore.collection(`Cuentas/${user.uid}/terceros`).valueChanges().subscribe( cuentas => {
    ...
    if(this.destino == this.numero) {
      ...
      if(this.monto < this.cantidad) {
        ...
        this.FireAuth.authState.subscribe( user => {
          ...
          this.FireStore.doc(`Cuentas/${user.uid}/terceros/${this.destino}`).valueChanges().subscribe((datosDestino:any)=>{
            console.log(datosDestino.monto);
            if(this.c == 0) {
              this.sumar = datosDestino.monto + this.cantidad
              this.FireStore.doc(`Cuentas/${user.uid}/terceros/${this.destino}`).update({
                  monto: this.sumar
              })
            }this.c++
